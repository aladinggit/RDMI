...
public:
  ...
  struct ibv_mr * reg_mem_exp_range(void *buf, size_t size){
      struct ibv_exp_reg_mr_in in = {0};
      in.pd = this->pd;
      in.addr = buf;
      in.length = size;
//      in.exp_access = IBV_EXP_ACCESS_REMOTE_WRITE | IBV_EXP_ACCESS_REMOTE_READ | IBV_EXP_ACCESS_REMOTE_ATOMIC | IBV_EXP_ACCESS_LOCAL_WRITE  | IBV_EXP_DEVICE_PHYSICAL_RANGE_MR;
      in.exp_access =  IBV_ACCESS_REMOTE_WRITE | IBV_ACCESS_REMOTE_READ | IBV_ACCESS_REMOTE_ATOMIC | IBV_ACCESS_LOCAL_WRITE  |IBV_EXP_ACCESS_PHYSICAL_ADDR;
      return ibv_exp_reg_mr(&in);
  }

  struct ibv_mr * reg_mem_exp_full(){
      struct ibv_exp_reg_mr_in in = {0};  
      in.pd = this->pd;
      in.addr = NULL;
      in.length = 0;
